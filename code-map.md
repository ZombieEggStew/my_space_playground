...
| `module_laser_gun.gd` | `LaserModule` | 激光机炮(P3 补 class_name):左右炮口交替、射速 Timer、过热停火、按热量加伤、`set_firing(bool)` 命令、`spawn_bullet()`;射击方向/散布基完全由 aim 模块给出(决策 #33,不自持相机,缺 aim 机头直射降级);ModulesManager 缓存 `get_laser_module()` |
| `aim/module_aim_mechanics.gd` | `AimMechanicsModule` | **共享瞄准力学(P5,决策 #11/#20)**:吸收原 predict——`solve_intercept_time` + `get_predicted_aim_data(target, bullet_speed)`(只算 3D 预测点/拦截时间,投影归 aim_view,决策 #18)+ `get_aim_direction_from_crosshair`(激光用,缺相机降级机头朝向)+ `get_aim_basis_from_crosshair`(决策 #33:完整射击 Basis,laser 不自持相机);`locked_target` 为"当前值",由 selection 推入 |
