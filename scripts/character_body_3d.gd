...
#TO DO : 子弹 实例池
#TO DO : 辅助瞄准；1级，鼠标靠近目标周围减速；2级：自动瞄准
...
#TO DO :
#TO DO : 我也做过类似测试。3d场景，每个角色10个组件左右，如果和作者一样设置的话，确实1000+角色之后就没法玩了。但是后来我发现很多节点的process其实是可以关掉的，因为这些节点并不需要逐帧更新。关掉之后帧率会得到巨大的提升，1000+角色也可以流畅运行。具体数据我不大记得了，好像都有100+帧率。注意:并不是enableprocess=true,好像是这么写，我忘了，这只是不运行process方法，而是processmode=disable，好像是这么写。我猜这样这个节点就不会被引擎遍历到，也就节省了大块性能。
#TO DO :
#TO DO : 参考 鸣潮 演出、特效
#TO DO : 意义不明/意义明确的滚转
#TO DO :
#TO DO : 敌人咬尾时使用PID控制器来调整速度和转向，以保持在玩家的后方。
#TO DO :每种颜色由白色和渐变色组成，白色区域控制显示纯色的区域，渐变灰色根据每个像素的灰度决定纯色显示概率（显示或者透明）
#TO DO :
#TO DO : 根据BV1uFANeFEMG的教程，修改星云颜色过渡区域
#TO DO : 
